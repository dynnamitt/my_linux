 ...
 * schema-driven.ts
 *
 * Reads a JSON Schema file and scaffolds a standalone editor package
 * into packages/<name>-editor/.
 *
 * Uses the same IR, builders, and templates as the TypeScript-driven
 * scaffolder (types-driven.ts + scaffold-v2/).
 *
 ...
 *
 * Usage: node --experimental-strip-types schema-driven.ts schemas/vt-demo.json
 */
...
import { parse, type ParseResult } from "./parse-schema.ts";
import {
...
if (!schemaPath) {
  console.error("Usage: node --experimental-strip-types schema-driven.ts <schema.json>");
  process.exit(1);
}
...
async function rescaffold(): Promise<void> {
  console.log(`Re-scaffolding @entur/${pkgName} from updated schema...\n`);
...
if (!isRescaffold) {
  console.log(`Scaffolding @entur/${pkgName} from ${schemaPath}...\n`);
